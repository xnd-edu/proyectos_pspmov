...
class GetReindeerByIdAdminUseCase @Inject constructor(
    private val reindeerRepository: ReindeerRepository,
    private val tokenManager: TokenManager
) {
    suspend operator fun invoke(id: Int): Result<Reindeer> {
        // Verificar que el usuario sea ADMIN
        if (!tokenManager.isAdmin()) {
            return Result.failure(NetworkError.Forbidden())
        }

        return reindeerRepository.getReindeerByIdAdmin(id)
    }
}
