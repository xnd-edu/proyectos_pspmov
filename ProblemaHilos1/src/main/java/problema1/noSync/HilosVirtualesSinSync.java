...
public class HilosVirtualesSinSync {
    private static final Logger logger = LoggerFactory.getLogger(HilosVirtualesSinSync.class);

    private ContadorVisitas contadorVisitas;
    private int visitasEsperadas;

    private final Random random = new Random();

    public HilosVirtualesSinSync(ContadorVisitas contadorVisitas) {
    ...
    public void ejecutarPrueba() {
        ...
        for (int i = 0; i < visitasEsperadas; i++) {
            ...
            Thread hiloVirtual = Thread.ofVirtual().start(() -> {
                ...
                contadorVisitas.incrementarVisita(false);
            });
        ...
        System.out.printf("Visitas contadas: %d %s%n", contadorVisitas.getVisitas(false), (contadorVisitas.getVisitas(false) == visitasEsperadas ? "✅ CORRECTO" : "❌ INCORRECTO"));
